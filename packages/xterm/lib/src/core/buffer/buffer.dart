...
class Buffer {
  ...
  void scrollDown(int lines) {
    // Avoid adopting the same BufferLine instance into multiple slots.
    // Doing that can detach a line that was already moved (xterm.dart #222).
    final regionLen = absoluteMarginBottom - absoluteMarginTop + 1;
    if (regionLen <= 0 || lines <= 0) return;
    final count = lines.clamp(0, regionLen);
    if (count == 0) return;

    for (var i = regionLen - 1; i >= count; i--) {
      final srcIndex = absoluteMarginTop + i - count;
      final dstIndex = absoluteMarginTop + i;
      final moving = this.lines[srcIndex];
      this.lines[srcIndex] = _newEmptyLine();
      this.lines[dstIndex] = moving;
    }

    // Ensure the newly exposed top region is clean.
  ...
  void scrollUp(int lines) {
    ...
    for (var i = 0; i < regionLen - count; i++) {
      final srcIndex = absoluteMarginTop + i + count;
      final dstIndex = absoluteMarginTop + i;
      final moving = this.lines[srcIndex];
      this.lines[srcIndex] = _newEmptyLine();
    ...
    // Ensure the newly exposed bottom region is clean.
    for (var i = regionLen - count; i < regionLen; i++) {
      this.lines[absoluteMarginTop + i] = _newEmptyLine();
    }
  }
